...
function saveCapsule(ds){var note=document.getElementById('capsule-note').value;var mood=S._moodPick||(S.capsuleData&&S.capsuleData.mood?S.capsuleData.mood:'');S.capsuleData={mood:mood,note:note};await api('/api/capsule',{method:'POST',body:JSON.stringify({date:ds,mood:mood,note:note})});try{var ma=await api('/api/mood/analysis');S.moodAnalysis=ma;showMoodAdvice()}catch(e){}loadMoodMap()}
